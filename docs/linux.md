...
**Manual binary:**
```bash
# Download anthropool-linux-amd64 or anthropool-linux-arm64 from releases
chmod +x anthropool-linux-amd64
...
If you used the shell installer or placed the binary at `~/.local/bin/anthropool` and the command is not found:
...
**If hooks fail to find `anthropool`** (Claude Code hooks run in a non-interactive shell):
...
WSL behaves like Linux. Use the Linux npm install or the shell installer inside WSL — do not use the Windows anthropool binary from within WSL.
...
On Linux, anthropool stores credentials in plain files under `~/.local/share/anthropool/accounts/` with `0600` permissions (owner read/write only). No keychain daemon is required.
