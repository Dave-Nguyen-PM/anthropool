...
//     anthropool must write here to actually change the active account.
//     macOS:     Keychain, generic password, service "Claude Code-credentials".
//     Linux/Win: File at ~/.claude/.credentials.json, mode 0600.
//
//   - "Backup" credentials are anthropool's per-account stash. On macOS/Windows
//     they live in the OS keystore under our own service name "anthropool-backup"
...
// path so anthropool never needs Claude Code to be running in order to obtain
...
// followed by `anthropool add` finds the credentials in the expected place.
const macKeychainService = "Claude Code-credentials"

// backupKeyringService is anthropool's own namespace inside the OS keystore on
// macOS/Windows. Distinct from claude-swap's "claude-code" so a user who
// has both tools installed sees no overlap.
const backupKeyringService = "anthropool-backup"
