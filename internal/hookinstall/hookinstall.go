// Package hookinstall manages anthropool's entries in Claude Code's
...
// touches entries it can identify as anthropool's own (by the literal
// substring "anthropool " or "/anthropool " in the command). Everything else is
...
// AnthropoolBinary is the literal command we install in settings.json. We use
// the bare name "anthropool" so it resolves through PATH; setupCheckPATH warns
// the user if anthropool is not findable.
const AnthropoolBinary = "anthropool"

// hookSpec describes one hook canthropool registers.
...
// Install adds (or refreshes) anthropool's hook entries in settings.json.
...
// Installed reports whether all anthropool hook events are present in Claude's
// settings file. It is intentionally tolerant of older anthropool hook shapes:
// if an event contains a anthropool-owned command, setup has at least been run.
...
// Uninstall removes only anthropool's hook entries. Other tools' entries and
...
// VerifyOnPATH reports whether the literal AnthropoolBinary is resolvable via
// $PATH. The hook commands installed in settings.json rely on this;
// `anthropool setup` calls VerifyOnPATH and surfaces a warning if it fails.
func VerifyOnPATH() (string, error) {
	resolved, err := exec.LookPath(AnthropoolBinary)
	if err != nil {
		return "", fmt.Errorf("`%s` not found on PATH — install anthropool first or add ~/.local/bin to PATH", AnthropoolBinary)
	}
	return resolved, nil
}
...
// upsertHook adds (or replaces) a anthropool entry under the given event key.
...
func upsertHook(hooks map[string][]json.RawMessage, s hookSpec) bool {
	...
	for _, e := range existing {
		if isAnthropoolEntry(e) {
			hadAnthropool = true
			// Replace stale anthropool entry with the current desired form;
	...
	// "changed" if the anthropool entry was missing OR if its bytes differ
	// from the desired form. Recompute by checking against original.
	if !hadAnthropool {
		return true
	}
	for _, e := range existing {
		if isAnthropoolEntry(e) && string(e) == string(desiredJSON) {
...
// removeHook strips every anthropool entry from one event, deleting the event
...
func buildHookEntry(s hookSpec) map[string]interface{} {
	return map[string]interface{}{
		...
		"hooks": []map[string]interface{}{
			{
				"type":    "command",
				"command": fmt.Sprintf("%s hook %s", AnthropoolBinary, s.Subcmd),
				"timeout": s.Timeout,
			},
		},
	}
...
// isAnthropoolEntry returns true if the JSON-encoded entry contains "anthropool "
// (with trailing space) or "/anthropool " — the two literal forms the
// command field can take.
func isAnthropoolEntry(e json.RawMessage) bool {
	s := string(e)
	return strings.Contains(s, `"`+AnthropoolBinary+` `) || strings.Contains(s, `/`+AnthropoolBinary+` `)
}
