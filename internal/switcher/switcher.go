...
// live storage and registers it in anthropool. If the account is already
...
// token (`claude login` again, then `anthropool add`).
...
func SwitchTo(identifier string) (from, to store.Account, err error) {
	...
	if err := state.Save(); err != nil {
		...
		// loudly. The next anthropool run will re-derive active from .claude.json.
...
// logged in to Claude Code, regardless of whether it's managed by anthropool.
