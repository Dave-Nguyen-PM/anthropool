...
// (https://api.anthropic.com/api/oauth/usage) so anthropool can show users
...
const (
	...
	envEndpoint      = "ANTHROPOOL_USAGE_ENDPOINT"
	envBetaHeader    = "ANTHROPOOL_USAGE_BETA"
...
// returned 401 (so the user knows to `claude login` and `anthropool add`
// again).
...
var ErrTokenExpired = fmt.Errorf("usage: token expired (re-login and `anthropool add`)")
