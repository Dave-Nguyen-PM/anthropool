...
// unless config.UpdateCheck.Enabled is true; anthropool makes no network call
// out of the box.
//
// The check hits the GitHub Releases API for the Dave-Nguyen-PM/anthropool
...
// configured cadence regardless of how often the user runs anthropool.
package updater
...
const (
	defaultRepo    = "Dave-Nguyen-PM/anthropool"
	envRepo        = "ANTHROPOOL_RELEASE_REPO"
...
type Result struct {
	Current string    // version anthropool was built as, no leading "v"
...
func fetchLatest(current string) (Result, error) {
	...
	req.Header.Set("User-Agent", "anthropool/"+stripV(current))
...
// build metadata are ignored — sufficient for anthropool's release policy
