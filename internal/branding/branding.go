// Package branding holds the anthropool ASCII banner shown at install / setup
// time. Centralising the literal here means install.sh, the npm
// postinstall, and `anthropool setup` can never drift from each other.
//
...
// terminals that don't auto-detect UTF-8 — `anthropool setup` falls back to
// a plain "anthropool" header when stdout looks like it can't render the
// glyphs.
package branding
...
const FallbackBanner = "" +
	"  ___  _  _ _____ _  _ ___  ___  ___  ___  _   \n" +
	" / _ \\| \\| |_   _| || | _ \\/ _ \\| _ \\/ _ \\| |  \n" +
	"| (_) | .` | | | | __ |   / (_) |  _/ (_) | |__\n" +
	" \\___/|_|\\_| |_| |_||_|_|_\\\\___/|_|  \\___/|____|\n"

// Tagline is the one-line strapline shown under the banner.
const Tagline = "ANTHROPOOL: Run multiple Claude Code Pro/Max accounts as one"
...
// unless the user explicitly opted in via $ANTHROPOOL_BANNER=unicode. The
...
func canRenderUnicode() bool {
	if v := os.Getenv("ANTHROPOOL_BANNER"); v != "" {
