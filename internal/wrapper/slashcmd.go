...
// SlashSwitch implements `anthropool __slash-switch <target>`, the body the
...
func SlashSwitch(target string, w io.Writer) error {
	if os.Getenv(envWrapped) != "1" {
		return errors.New("/switch requires anthropool as the entry point — start your session with `anthropool` instead of `claude`")
	}

	pidStr := os.Getenv(envWrapperPID)
	if pidStr == "" {
		return errors.New("ANTHROPOOL_WRAPPER_PID not set; cannot route switch")
	}
	wrapperPID, err := strconv.Atoi(pidStr)
	if err != nil || wrapperPID <= 0 {
		return fmt.Errorf("invalid ANTHROPOOL_WRAPPER_PID: %q", pidStr)
	}
	...
	if len(state.Accounts) < 2 {
		return errors.New("need at least two managed accounts — run `anthropool add` after logging into another account")
	}
	if target != "" {
		...
		if resolved.Slot == state.ActiveSlot {
			fmt.Fprintf(w, "anthropool: already on %s, nothing to do\n", resolved.Email)
			return nil
		}
...
// `anthropool force-switch [target]`; the active wrapper sees the same signal
// that /switch would have written.
func ForceSwitch(target string, w io.Writer) error {
	b, err := os.ReadFile(paths.ClaudePIDFile())
	if err != nil {
		if os.IsNotExist(err) {
			return errors.New("no active anthropool wrapper found — start Claude with `anthropool` first")
		}
		return err
	}
	...
	if target != "" {
		...
		fmt.Fprintf(w, "anthropool: forcing switch to %s in active session\n", resolved.Email)
	} else {
		fmt.Fprintln(w, "anthropool: forcing rotation in active session")
	}
	return signals.Write(wrapperPID, signals.SwitchRequested, signals.SwitchRequestedPayload{
		Target:    target,
		Timestamp: time.Now().UTC(),
