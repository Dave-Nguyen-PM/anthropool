...
const (
	...
	envClaudeBin  = "ANTHROPOOL_CLAUDE_BIN"
...
// on the first launch (anything after `anthropool`); subsequent launches are
...
func Run(claudeBin string, argv []string, w io.Writer) (int, error) {
	...
	if err := writeWrapperPID(pid); err != nil {
		fmt.Fprintf(w, "anthropool: warning: cannot publish wrapper pid: %v\n", err)
	}
	...
	for {
		...
		if err != nil {
			fmt.Fprintf(w, "anthropool: %v — staying on current account\n", err)
			return exitCode, nil
		}

		from, to, swapErr := switcher.SwitchTo(target)
		if swapErr != nil {
			fmt.Fprintf(w, "anthropool: switch failed: %v\n", swapErr)
			return 1, swapErr
		}
		...
		// session, but doing it here gives `anthropool list` correct data
		// immediately.
		...
		if canResume {
			...
			case history.TriggerRateLimit:
				fmt.Fprintf(w, "anthropool: rate limit on %s → swapped to %s, resuming…\n", from.Email, to.Email)
			case history.TriggerManual:
				fmt.Fprintf(w, "anthropool: %s → %s, resuming…\n", from.Email, to.Email)
			default:
				fmt.Fprintf(w, "anthropool: %s → %s (%s), resuming…\n", from.Email, to.Email, p.reason)
			}
			...
			fmt.Fprintf(w, "anthropool: switched to %s — now active\n", to.Email)
			currentArgv = argv
		}
	}
}
...
func step(
	...
	if b, ok, _ := signals.Read(wrapperPID, signals.RateLimited); ok {
		...
		if cfg.AutoSwitchOnRateLimit {
			...
			fmt.Fprintln(w, "anthropool: rate-limit hook fired but auto_switch_on_rate_limit is off; staying on current account")
		}
	}
...
func gracefulExit(cmd *exec.Cmd, w io.Writer) {
	...
	for {
		select {
		case <-deadline.C:
			fmt.Fprintln(w, "anthropool: claude did not exit cleanly, terminating…")
			_ = cmd.Process.Kill()
			return
		case <-tick.C:
