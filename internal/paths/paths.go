// Package paths centralises every on-disk location anthropool reads or writes,
...
//     anthropool never invents these — it follows whatever Claude Code itself uses.
//   - anthropool's own state, under a backup root that follows XDG on Linux and
//     falls back to $HOME/.anthropool elsewhere.
package paths
...
func Home() string {
	...
	if err != nil {
		panic("anthropool: cannot resolve home directory: " + err.Error())
	}
	return h
}
...
// BackupRoot is anthropool's own data directory.
//
// On Linux it follows XDG: $XDG_DATA_HOME/anthropool, defaulting to
// $HOME/.local/share/anthropool. On macOS/Windows we keep things simple at
// $HOME/.anthropool. No legacy migration code — anthropool is unreleased before
...
func BackupRoot() string {
	if Detect() == Linux {
		...
		return filepath.Join(xdg, "anthropool")
	}
	return filepath.Join(Home(), ".anthropool")
}
