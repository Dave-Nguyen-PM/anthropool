// Package hooks implements the bodies of `anthropool hook stop`,
// `anthropool hook session-start`, and `anthropool hook rate-limit`.
//
...
// emit a signal file the anthropool wrapper polls for.
//
// All three are gated by the ANTHROPOOL_WRAPPED env var. When unset (the user
// is running `claude` directly, not under `anthropool`), the hook silently
// no-ops with exit 0 — so installed hooks are harmless when anthropool is not
// the parent process.
package hooks
...
import (
	...
	"github.com/Dave-Nguyen-PM/anthropool/internal/signals"
	"github.com/Dave-Nguyen-PM/anthropool/internal/store"
	"github.com/Dave-Nguyen-PM/anthropool/internal/strategy"
	"github.com/Dave-Nguyen-PM/anthropool/internal/switcher"
	"github.com/Dave-Nguyen-PM/anthropool/internal/usage"
)

const (
	envWrapped    = "ANTHROPOOL_WRAPPED"
	envWrapperPID = "ANTHROPOOL_WRAPPER_PID"
...
// UserPromptSubmit is `anthropool hook prompt-submit`. It intercepts anthropool's
...
func UserPromptSubmit(stdin io.Reader, stdout io.Writer) error {
	...
	if handled, err := handleAnthropoolPromptCommand(prompt, stdout); handled || err != nil {
		return err
	}
...
func handleAutoSwitchPrompt(prompt string, stdout io.Writer) (bool, error) {
	...
	writePromptBlock(stdout, "anthropool: "+reason+"\nanthropool: switching accounts and replaying your prompt after resume...")
	return true, nil
}
...
func handleAnthropoolPromptCommand(prompt string, stdout io.Writer) (bool, error) {
	if !strings.HasPrefix(prompt, "/anthropool:") {
		return false, nil
	}
	fields := strings.Fields(strings.TrimPrefix(prompt, "/anthropool:"))
	if len(fields) == 0 {
		writePromptBlock(stdout, "anthropool: missing command after /anthropool:")
		return true, nil
	}
	...
	case "switch":
		target := strings.TrimSpace(strings.TrimPrefix(prompt, "/anthropool:switch"))
		writePromptSwitch(target, stdout)
		return true, nil
	case "list":
		...
		if err != nil {
			writePromptBlock(stdout, "anthropool: "+err.Error())
			return true, nil
		}
	...
	case "usage-refresh":
		...
		return true, nil
	case "help":
		writePromptBlock(stdout, "anthropool commands: /switch, /anthropool:switch, /anthropool:add, /anthropool:list, /anthropool:status, /anthropool:support, /anthropool:remove, /anthropool:config, /anthropool:usage-refresh")
		return true, nil
	default:
		writePromptBlock(stdout, "anthropool: unknown /anthropool command "+fields[0])
		return true, nil
	}

	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, "anthropool", args...)
	cmd.Env = os.Environ()
	...
	if ctx.Err() == context.DeadlineExceeded {
		text = "anthropool: command timed out"
	} else if err != nil && text == "" {
		text = "anthropool: " + err.Error()
	} else if err != nil {
		text = text + "\n" + "anthropool: command failed: " + err.Error()
	} else if text == "" {
		text = "anthropool: done"
	}
	writePromptBlock(stdout, text)
	return true, nil
}
...
func writePromptSwitch(target string, stdout io.Writer) {
	...
	reason := "anthropool: switching accounts..."
	if strings.TrimSpace(target) != "" {
		reason = "anthropool: switching accounts to " + strings.TrimSpace(target) + "..."
	}
...
func promptSwitchHasTarget() (bool, string) {
	...
	text, err := renderPromptUsage(false)
	if err != nil {
		return false, "anthropool: no usable accounts available; all managed accounts are exhausted or need login"
	}
	return false, text
}
...
func renderPromptUsage(refresh bool) (string, error) {
	...
	if len(state.Accounts) == 0 {
		return "ANTHROPOOL\n\nNo managed accounts yet. Use /anthropool:add after logging in.", nil
	}
...
func renderPromptConfig(args []string) (string, error) {
	...
	case "show", "keys":
		...
		b.WriteString("└─────────────────────────────┴──────────────────────┴────────────────────────────────────────────┘\n\n")
		b.WriteString("SET     /anthropool:config set [key] [value]\n")
		b.WriteString("EDIT    run `anthropool config edit` in your terminal for the interactive settings UI\n")
		b.WriteString("EXAMPLE  /anthropool:config set strategy.kind balanced")
		return b.String(), nil
	case "edit":
		return ":: C U X   S E T T I N G S ::\n\nInteractive settings editor is terminal-only.\nRun this outside Claude:\n\n  anthropool config edit", nil
	case "set":
		if len(args) != 3 {
			return "", fmt.Errorf("usage: /anthropool:config set <key> <value>")
		}
		c, err := config.Load()
		if err != nil {
			return "", err
	...
	default:
		return "", fmt.Errorf("usage: /anthropool:config show | keys | set <key> <value>")
	}
}

func renderPromptSupport() string {
	return "ANTHROPOOL SUPPORT\n\nSupport anthropool development:\nhttps://support.dave.com"
}
...
func renderAccountRow(slot int, email string, u usage.AccountUsage, stateLabel string) string {
	...
	if u.TokenExpired {
		line += fmt.Sprintf("│ %-96s │\n", "TOKEN EXPIRED: RE-LOGIN AND RUN /anthropool:add")
	}
	return line
}
...
// Stop is `anthropool hook stop`. Always emits a Stopped signal in wrapped
...
// SessionStart is `anthropool hook session-start`. We capture the session ID
...
// RateLimit is `anthropool hook rate-limit`. Claude Code routes generic
...
func wrapperPID() (int, error) {
	...
	if v == "" {
		return 0, errors.New("hook: ANTHROPOOL_WRAPPER_PID not set")
	}
	pid, err := strconv.Atoi(v)
	if err != nil || pid <= 0 {
		return 0, fmt.Errorf("hook: bad ANTHROPOOL_WRAPPER_PID %q", v)
	}
	return pid, nil
}
