// Package config persists user preferences for anthropool at
// $XDG_CONFIG_HOME/anthropool/config.json (or $HOME/.config/anthropool/config.json
...
// UpdateCheckConfig controls whether anthropool checks for newer releases on
// the GitHub repo. Off by default so installing anthropool never causes a
// network call the user didn't ask for. `anthropool setup` offers to enable
// it interactively. The cadence cap means we ping GitHub at most once
// per CadenceHours regardless of how often `anthropool` runs.
...
// `anthropool config keys`. Default is the value as a string for display
...
// see in `anthropool config keys` — keep stable so docs and screenshots age
// well.
func Keys(c Config) []KeyInfo {
	return []KeyInfo{
		...
		},
		{
			...
			Description: "check GitHub for newer anthropool releases on startup",
...
func filePath() string {
	if v := os.Getenv("ANTHROPOOL_CONFIG_FILE"); v != "" {
		return v
	}
	...
	if base == "" {
		...
		if err != nil {
			...
			return filepath.Join(os.TempDir(), "anthropool-config.json")
		}
		base = filepath.Join(home, ".config")
	}
	return filepath.Join(base, "anthropool", "config.json")
}
